...
Outputs:
- [IDI_Sandpit].[DL-MAA2020-01].[defn_employed_spell]
...
/* Clear existing view */
DROP VIEW IF EXISTS [DL-MAA2020-01].[tmp_employed_spell_staging];
GO

/* Create staging */
CREATE VIEW [DL-MAA2020-01].[tmp_employed_spell_staging] AS
SELECT snz_uid
...
AND [ir_ems_return_period_date] BETWEEN '2011-12-01' AND '2019-01-31';
GO

/* Condensed spells */
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2020-01].[defn_employed_spell];
GO
...
spell_starts AS (
	...
	FROM [IDI_UserCode].[DL-MAA2020-01].[tmp_employed_spell_staging] s1
	WHERE [start_date] <= [end_date]
	AND NOT EXISTS (
		SELECT 1
		FROM [IDI_UserCode].[DL-MAA2020-01].[tmp_employed_spell_staging] s2
...
spell_ends AS (
	...
	FROM [IDI_UserCode].[DL-MAA2020-01].[tmp_employed_spell_staging] t1
	WHERE [start_date] <= [end_date]
	AND NOT EXISTS (
		SELECT 1 
		FROM [IDI_UserCode].[DL-MAA2020-01].[tmp_employed_spell_staging] t2
...
INTO [IDI_Sandpit].[DL-MAA2020-01].[defn_employed_spell]
FROM spell_starts s
...
/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[defn_employed_spell] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2020-01].[defn_employed_spell] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
