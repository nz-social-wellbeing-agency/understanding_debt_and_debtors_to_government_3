...
Output:
- [IDI_UserCode].[DL-MAA2020-01].[defn_taxable_income_calendar_year]
- [IDI_UserCode].[DL-MAA2020-01].[defn_nontaxable_T2_benefit_income]
- [IDI_UserCode].[DL-MAA2020-01].[defn_nontaxable_T3_benefit_income]
- [IDI_Sandpit].[DL-MAA2020-01].[defn_nontaxable_wff_income]
...
DROP VIEW IF EXISTS [DL-MAA2020-01].[defn_taxable_income_calendar_year];
GO

CREATE VIEW [DL-MAA2020-01].[defn_taxable_income_calendar_year] AS
SELECT [snz_uid]
	,[snz_ird_uid]
...
FROM [IDI_Clean_20211020].[data].[income_cal_yr]
WHERE [inc_cal_yr_year_nbr] BETWEEN 2012 AND 2020
GO
...
DROP VIEW IF EXISTS [DL-MAA2020-01].[defn_nontaxable_T2_benefit_income];
GO

CREATE VIEW [DL-MAA2020-01].[defn_nontaxable_T2_benefit_income] AS
SELECT [snz_uid]
	,[msd_ste_start_date]
	,[msd_ste_end_date]
...
FROM  [IDI_Clean_20211020].[msd_clean].[msd_second_tier_expenditure]
WHERE YEAR([msd_ste_end_date]) BETWEEN 2012 AND 2020;
GO
...
DROP VIEW IF EXISTS [DL-MAA2020-01].[defn_nontaxable_T3_benefit_income];
GO

CREATE VIEW [DL-MAA2020-01].[defn_nontaxable_T3_benefit_income] AS
SELECT snz_uid
...
AND YEAR([msd_tte_decision_date]) BETWEEN 2012 AND 2020;
GO
...
DROP VIEW IF EXISTS [DL-MAA2020-01].[tmp_wff];
GO
...
FROM [IDI_Clean_20211020].[wff_clean].[fam_return_dtls]
WHERE YEAR([wff_frd_return_period_date]) BETWEEN 2012 AND 2020
GROUP BY  [snz_uid], [wff_frd_return_period_date]
GO

-- add the negative wff payment back, and deduct it from previous wff payment
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2020-01].[defn_nontaxable_wff_income];
GO

 SELECT *
       ,wff_pmt + neg_adj + ISNULL(lead_neg_deduct,0) AS wff_pmt_neg_adj
INTO [IDI_Sandpit].[DL-MAA2020-01].[defn_nontaxable_wff_income]
FROM (
	SELECT *
...
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[defn_nontaxable_wff_income] (snz_uid);
GO
/* Compress to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2020-01].[defn_nontaxable_wff_income] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
